...
def handle_message_events(body, say, logger):
    try:
        ...
        prompt = (
            ...
            f"- In the policy, find the section that starts with exactly '{employment_type}:' and return ONLY that answer if available.\n"
            f"- If the policy explicitly says 'Please contact HR', reply exactly with that wording.\n"
            f"- Keep the response consolidated if word count is above 150 words.\n"
            f"- If no answer exists, say: 'I have limited knowledge on this, please contact HR for clarification.'\n"
            f"- Use short bullet points or concise sentences.\n"
            f"- Never cut off mid-sentence.\n"
            f"- Maximum 10 bullet points or 150 words.\n"
            f"- Phrase answers naturally, not verbatim.\n"
        )
